/**
 * @file src/pages/Movies/MoviesPage.jsx
 * @description Browsing page for movies with infinite scroll and genre filtering.
 */
...
export default function MoviesPage() {
  ...
  // Fetch Genres
  const { data: genresData } = useFetch(() => movieService.getGenres());
  ...
  const fetchMovies = useCallback(async (pageNum, genreId, isNewSearch = false) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = genreId 
        ? await movieService.getByGenre(genreId, pageNum)
        : await movieService.getPopular(pageNum);
  ...
  useEffect(() => {
    if (isIntersecting && hasMore && !isLoading) {
      ...
      setPage(nextPage);
      fetchMovies(nextPage, selectedGenre, false);
    }
  }, [isIntersecting, hasMore, isLoading, page, selectedGenre, fetchMovies]);
    ...
    <div className="min-h-screen px-[clamp(1rem,5vw,4rem)] pt-6 pb-20" style={{ background: 'var(--bg-base)' }}>
      ...
      >
        <AnimatePresence mode="popLayout">
          {items.map((item, idx) => (
            ...
            >
              <MovieCard item={item} mediaType={MEDIA_TYPE.MOVIE} size="lg" />
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
      ...
      {error && (
        <div className="py-20 text-center text-red-500">
          ...
          <button 
            onClick={() => fetchMovies(page, selectedGenre, page === 1)}
